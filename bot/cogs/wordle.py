...
def available_letters(guesses: str) -> str:
    """Get string of alphabet with unavailable letters with
    lines through them and all bold"""
    letters = ""
...
def create_embed(guesses: str, grid: str) -> discord.Embed:
    ...
    if len(guesses) != 0:
        embed.add_field(name="Guesses:", value=formatted_guesses,
                        inline=False)
    embed.add_field(
        name="Available Letters:", value=available_letters(guesses),
        inline=False)
    embed.set_footer(
        text="Guess a five letter word with ! prefix (e.g. !hello)")
    return embed
...
class Wordle(commands.Cog, name='Wordle'):
    ...
    async def wordle(self, ctx):
        ...
        embed.add_field(
            name="You Win!",
            value=f"You guessed the correct word '{word}'!")
