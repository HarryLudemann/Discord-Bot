...
class Poll(Cog, name='Poll'):
    ...
    class Poll(discord.ui.View):
        ...
        class GenericButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                try:
                    ...
                    # add custom_id and empty list to votes dict if not already there
                    ...
                    await self.view.results_message.edit(embed=self.view.get_results())
        ...
        def get_results(self) -> discord.Embed:
            ...
            embed = discord.Embed(title=self.question, color=discord.Color.blurple())
            for i, option in enumerate(self.options):
                # be careful of zero division error
                if self.total_votes == 0:
                    embed.add_field(name=option, value=f"{self.results[str(i)]} votes (0%)", inline=False)
                else:
                    embed.add_field(name=option, value=f"{self.results[str(i)]} votes ({round(self.results[str(i)]/self.total_votes*100, 2)}%)", inline=False)
            return embed
